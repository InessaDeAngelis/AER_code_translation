...
*full specification
eststo: reghdfe is_under_city_state_map_tot_alt $add_treatment, ///
 cluster($cluster_level) absorb($baseline_FE)
$add_y

*alternative outcome
eststo: reghdfe building_alt $add_treatment, ///
 cluster($cluster_level) absorb($baseline_FE)
$add_y

*New states
eststo: reghdfe city_state_map_tot_new_alt $add_treatment, ///
 cluster($cluster_level) absorb($baseline_FE)
$add_y

*Expanding states
eststo: reghdfe city_state_map_tot_cont_alt $add_treatment, ///
 cluster($cluster_level) absorb($baseline_FE)
$add_y

esttab  using "$project_output_path\RA21.tex", ///
b(2) se(2) se ///
nonumber label ///
