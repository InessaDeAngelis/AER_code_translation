...
*full specification
eststo: reghdfe is_under_city_state_map_tot $add_treatment_2km, ///
 cluster($cluster_level) absorb($baseline_FE)
$add_y

*alternative outcome
eststo: reghdfe building $add_treatment_2km, ///
 cluster($cluster_level) absorb($baseline_FE)
$add_y

*New states
eststo: reghdfe is_under_city_state_map_tot_new $add_treatment_2km, ///
 cluster($cluster_level) absorb($baseline_FE)
$add_y

*Expanding states
eststo: reghdfe is_under_city_state_map_tot_cont $add_treatment_2km, ///
 cluster($cluster_level) absorb($baseline_FE)
$add_y

esttab  using "$project_output_path\RA13.tex", ///
b(2) se(2) se ///
nonumber label ///
