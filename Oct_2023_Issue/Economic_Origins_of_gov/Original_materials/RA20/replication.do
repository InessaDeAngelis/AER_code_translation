...
*full specification
eststo: reghdfe is_under_city_state_map_tot_15km $add_treatment, ///
 cluster($cluster_level) absorb($baseline_FE)
$add_y

*alternative outcome
eststo: reghdfe building_15km $add_treatment, ///
 cluster($cluster_level) absorb($baseline_FE)
$add_y

*New states
eststo: reghdfe is_under_city_state_map_totN15km $add_treatment, ///
 cluster($cluster_level) absorb($baseline_FE)
$add_y

*Expanding states
eststo: reghdfe is_under_city_state_map_totC15km $add_treatment, ///
 cluster($cluster_level) absorb($baseline_FE)
$add_y

esttab  using "$project_output_path\RA20.tex", ///
b(2) se(2) se ///
nonumber label ///
