...
epsilon_s=PARAM(:,5); epsilon_s([S+1:length(PARAM)])=[];    % drop elasticities for aggregated sectors: for initial sectors 5 and 4 elasticities are the same, while initial sectors 17:35 are non-tradable (also have the same elasticity)
...
mu_s=PARAM(:,6); mu_s([S+1:length(PARAM)])=[];    % drop elasticities for aggregated sectors: for initial sectors 5 and 4 elasticities are the same, while initial sectors 17:35 are non-tradable (also have the same elasticity)
