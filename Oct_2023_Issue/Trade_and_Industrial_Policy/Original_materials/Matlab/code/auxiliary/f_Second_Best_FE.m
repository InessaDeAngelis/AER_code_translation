function [ceq, Gains] = Second_Best_FE(X, N ,S, Yi3D, Ri3D, e_ik3D, sigma_k3D, lambda_jik3D, rik3D, mu_k3D, tjik_3D_app, id, case_id)
...
AUX7 = (1 + ( sigma_k3D - 1 ).*AUX2)./ ( 1  + Ratio.*( sigma_k3D - 1 ).*AUX2 );
...
subsidy = AUX7./(1-AUX2);
x_pred  = Ratio.*(1 + 1./((sigma_k3D-1).*(1-AUX2)))./(1+subsidy);
