function [ceq, Gains] = Third_Best_FE(X, N ,S, Yi3D, Ri3D, e_ik3D, sigma_k3D, lambda_jik3D, rik3D, mu_k3D, tjik_3D_app, id, case_id)
...
AUX7 = AUX3.*repmat(1-eye(N),[1 1 S]);
 Profit =  sum(sum(mu_k3D.* AUX3./(1+mu_k3D),3),2);    
 mu_avg = (1 +  Profit./sum(sum(AUX3./(1+mu_k3D),3),2));
 mu_avg_3D =  repmat(mu_avg, [1 N S]);
% mu_avg_3D =  ones(N,N,S);
Ratio = mu_avg_3D./(1+mu_k3D);
AUX8 = (1 + ( sigma_k3D - 1 ).*AUX2)./ ( 1  + Ratio.*( sigma_k3D - 1 ).*AUX2 );
Multiplier = AUX8(repmat(eye(N),[1 1 S])==1);   
AUX9 = ( AUX7.*( sigma_k3D - 1 ).*(1-AUX2) ) ./ repmat(sum(sum(AUX7,2),3), [1 1 S]) ;
tau = 1./sum(sum(AUX9,2),3);
AUX10 = repmat(permute(reshape(Multiplier.*((1 + repmat(tau,S,1))),N,S)', [3 2 1]), [N 1 1]);
  
t_pred = (1 + omega).* AUX10 - 1; 
