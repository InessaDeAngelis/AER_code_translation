function [ceq, Gains] = Third_Best_RE(X, N ,S, Yi3D, Ri3D, e_ik3D, sigma_k3D, lambda_jik3D, mu_k3D, tjik_3D_app, id, case_id)
...
AUX7 = (1+mu_k3D).*AUX3.*repmat(1-eye(N),[1 1 S]);
mu_avg = (1 +  Profit./(wi_h.*Ri3D(:,1,1)));
mu_avg_3D =  repmat(mu_avg, [1 N S]);
AUX4 = mu_avg_3D./(1+mu_k3D);
AUX8 = (1 + ( sigma_k3D - 1 ).*AUX2)./ ( 1  + AUX4.*( sigma_k3D - 1 ).*AUX2 );
