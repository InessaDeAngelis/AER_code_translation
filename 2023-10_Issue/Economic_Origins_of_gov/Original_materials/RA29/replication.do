...
space_reg Y X cutoff1 cutoff2 tablet period_lag_2_tr period_tr_tr, /// 
 xreg(2) coord(2) model(reghdfe, $baseline_FE) 
mat se = e(se)
scalar c_se = se[2, 1]

reghdfe tablet $add_treatment, ///
 cluster($cluster_level) absorb($baseline_FE)
$add_y
$add_p_value 
$rec_est

space_reg Y X cutoff1 cutoff2 admin_tablet period_lag_2_tr period_tr_tr, /// 
 xreg(2) coord(2) model(reghdfe, $baseline_FE) 
mat se = e(se)
scalar c_se = se[2, 1]

reghdfe admin_tablet $add_treatment, ///
 cluster($cluster_level) absorb($baseline_FE)
$add_y
$add_p_value 
$rec_est

space_reg Y X cutoff1 cutoff2 tax_indicator period_lag_2_tr period_tr_tr, /// 
 xreg(2) coord(2) model(reghdfe, $baseline_FE) 
mat se = e(se)
scalar c_se = se[2, 1]

reghdfe tax_indicator $add_treatment, ///
 cluster($cluster_level) absorb($baseline_FE)
$add_y
$add_p_value 
$rec_est

space_reg Y X cutoff1 cutoff2 tax_food_indicator period_lag_2_tr period_tr_tr, /// 
 xreg(2) coord(2) model(reghdfe, $baseline_FE) 
mat se = e(se)
scalar c_se = se[2, 1]

reghdfe tax_food_indicator $add_treatment, ///
 cluster($cluster_level) absorb($baseline_FE)
$add_y
$add_p_value 
$rec_est

esttab  using "$project_output_path\RA29.tex", ///
b(2) se(2) se ///
