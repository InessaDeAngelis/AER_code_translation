...
// STANDARD NK
// Preferemces
set_param_value('ggamma', ggammaV);           
set_param_value('h', hV);               
set_param_value('psi_L', psi_LV);              
set_param_value('bbeta', bbetaV);          

// Technology
set_param_value('Norm', NormV);             
set_param_value('nnu', nnuV);             
set_param_value('aalpha', aalphaV)          
alpha_k = nnu * aalpha;
alpha_l = nnu * (1-aalpha);
set_param_value('ddelta', ddeltaV);         
set_param_value('xxi', xxiV);              
set_param_value('Y_bd_sh', Y_bd_shV);          
set_param_value('A_nbd', A_nbdV);            
set_param_value('elast_invest', elast_investV);
kappa_I_nbd = 1/elast_invest;        
kappa_I_bd = 1/elast_invest;      


// Price stickiness and monetary policy
set_param_value('eps_PC', eps_PCV); 
set_param_value('Pi_SS', Pi_SSV);        
                           
set_param_value('rho_Rn', rho_RnV);          
set_param_value('phi_pi', phi_piV);         
set_param_value('phi_y', phi_yV);            
set_param_value('theta_PC', theta_PCV); 

// BANK PARAMETERS
set_param_value('L_to_S', L_to_SV);           
set_param_value('Iss_to_A', Iss_to_AV);

set_param_value('Eq_to_L', Eq_to_LV); 
LvgRatio = Eq_to_L * L_to_S/(1+L_to_S);

set_param_value('DGDP_ratio', DGDP_ratioV);

set_param_value('eps_D', eps_DV);
set_param_value('ben_D', ben_DV);
set_param_value('eps_L', eps_LV)
set_param_value('tau_S', tau_SV);   
set_param_value('rho_SS', rho_SSV);
set_param_value('lvg_elasticity', lvg_elasticityV);

kappa_L =  lvg_elasticity/4 * 1 / (1/rho_SS - 1/(rho_SS+0.0025))^2;
rho_eq = 1-Iss_to_A / LvgRatio;

// SHOCKS
set_param_value('rho_A', rho_AV);
set_param_value('rho_mp', rho_mpV);
set_param_value('rho_fg', rho_fgV);
set_param_value('rho_bbeta', rho_bbetaV);
