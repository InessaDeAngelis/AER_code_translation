// Version of the main dynare file to produce diff-in-diff figure (right panel of Figure 8)
// Only difference from Dyn_RIR_commented.mod is in the "shocks" 
// block (saved variables + shocks imported from 'shocks_RIR_lending.mat') 

var llambda R dRL rk_nbd rk_bd w EA_k L L_nbd L_bd K_nbd K_bd Y Y_bd Y_nbd C Invest Invest_nbdx Invest_bdx  Invest_nbd Invest_bd Q_nbd Q_bd nu_A nu_mp nu_fg Rn Pi P_I D RDn RLn Pi_D Pi_L LevCost Lo Eq_B CG S Q_S RS NII_B RL Divs ROE_annual bbeta_endog nu_bbeta LR_rate Rn_target Trule D_nd RLn_nd dRL_nd Pi_D_nd Pi_L_nd CG_nd Lo_nd S_nd Eq_B_nd NII_B_nd LevCost_nd RL_nd LR_rate_nd Divs_nd;
varexo eps_A eps_mp eps_fg eps_bbeta eps_target eps_Trule;

parameters bbeta h Norm   nnu aalpha alpha_l alpha_k ddelta A_nbd ggamma elast_invest kappa_I_nbd rho_A cchi psi_L rho_Rn phi_pi phi_y rho_mp rho_fg eps_PC theta_PC Pi_SS L_to_S Eq_to_L ben_D D_bar cchi_D D_max DGDP_ratio kappa_L eps_L eps_D delta_E tau_S rho_SS lvg_elasticity xxi A_bd kappa_I_bd Y_bd_sh Invest_bdx_SS Invest_nbdx_SS Invest_SS rho_bbeta rho_eq LvgRatio Iss_to_A delta_E_nd LvgRatio_nd rho_eq_nd rho_SS_nd kappa_L_nd Del_D;
...
load('params_diffdiff.mat', 'deposit_diff')

Del_D = deposit_diff;

D_nd_SS = D_SS - Del_D*(Eq_B_SS+D_SS);
Eq_B_nd_SS = Eq_B_SS + D_SS - D_nd_SS;
// Eq_B_nd_SS = (Eq_to_L_nd / (1 + 1/L_to_S)) / (1 - Eq_to_L_nd/(1 + 1/L_to_S)) * D_nd_SS;
Lo_nd_SS = Lo_SS;

Eq_to_L_nd = Eq_B_nd_SS/Lo_SS;
LvgRatio_nd = Eq_to_L_nd * L_to_S/(1+L_to_S);
...
// rho_SS_nd = Eq_to_L;
rho_SS_nd = Eq_to_L_nd;

Pi_D_nd_SS = (Rn_SS+ben_D-RDn_SS)*D_nd_SS/Pi_SS;
Pi_L_nd_SS = (RLn_SS-Rn_SS)*Lo_nd_SS/Pi_SS;

NII_B_nd_SS = (Rn_SS-1)/Pi_SS*Eq_B_nd_SS + Pi_D_nd_SS + Pi_L_nd_SS;

delta_E_nd = NII_B_nd_SS/(NII_B_nd_SS + Eq_B_nd_SS);
...
// NON-DEPOSIT-DEPENDENT BANKS
D_nd = D * (1 - Del_D*(Eq_to_L/(1+1/L_to_S-Eq_to_L)+1));
Pi_D_nd = (Rn+ben_D-RDn)*D_nd/Pi(+1);

RLn_nd = eps_L/(eps_L-1)*(Rn + dRL_nd);
Lo_nd = (RLn_nd/RLn)^(-eps_L) * Lo;
Pi_L_nd = (RLn_nd-Rn)*Lo_nd/Pi(+1);
dRL_nd = kappa_L_nd * max(0, Lo_nd/Eq_B_nd - 1/rho_SS_nd)^2;
LevCost_nd = dRL_nd * Lo_nd/Pi(+1);

S_nd = Eq_B_nd + D_nd - Lo_nd;
NII_B_nd = Pi_L_nd + Pi_D_nd + (Rn/Pi(+1)-1)*Eq_B_nd - LevCost_nd;

CG_nd = (RS - R(-1))*S_nd(-1);
Eq_B_nd = rho_eq_nd * (1-delta_E_nd) * (CG_nd + NII_B_nd(-1) + Eq_B_nd(-1)) + (1-rho_eq_nd) * steady_state(Eq_B_nd);

Divs_nd = CG_nd + NII_B_nd(-1) + Eq_B_nd(-1) - Eq_B_nd;

RL_nd = RLn_nd/Pi(+1);
LR_rate_nd = log(RL_nd) + log(RL_nd(+1)) + log(RL_nd(+2)) + log(RL_nd(+3));
...
// Non-deposit-dependent banks
D_nd = D_bar * (1 - Del_D*(Eq_B_SS/D_SS+1));
Pi_D_nd = (Rn+ben_D-RDn)*D_nd/Pi;

dRL_nd = 0 ;
Lo_nd = K_bd;
RLn_nd = eps_L/(eps_L-1) * Rn;
Pi_L_nd = (RLn_nd - Rn)/Pi*Lo_nd;
LevCost_nd = 0;

CG_nd = 0;
NII_B_nd = NII_B_nd_SS;
Eq_B_nd = (1-delta_E_nd)/delta_E_nd*NII_B_nd_SS;
S_nd = Eq_B_nd + D_nd - Lo_nd;

Divs_nd = NII_B_nd;

RL_nd = RLn_nd/Pi;
LR_rate_nd = 4 * log(RL_nd);
