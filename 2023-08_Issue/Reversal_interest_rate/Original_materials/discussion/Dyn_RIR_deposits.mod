// Modified model with different deposit spread (in Appendix)
// Main difference is positive pass-through of policy rate to deposit spread
...
parameters bbeta h Norm   nnu aalpha alpha_l alpha_k ddelta A_nbd ggamma elast_invest kappa_I_nbd rho_A cchi psi_L rho_Rn phi_pi phi_y rho_mp rho_fg eps_PC theta_PC Pi_SS L_to_S Eq_to_L ben_D D_bar cchi_D D_max DGDP_ratio kappa_L eps_L sp_D delta_E tau_S rho_SS lvg_elasticity xxi A_bd kappa_I_bd Y_bd_sh Invest_bdx_SS Invest_nbdx_SS Invest_SS rho_bbeta rho_eq LvgRatio Iss_to_A;
