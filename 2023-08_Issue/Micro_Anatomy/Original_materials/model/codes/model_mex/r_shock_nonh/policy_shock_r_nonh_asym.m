function [c_policy, a_policy,y_grid,a_grid] = policy_shock_r_nonh_asym(par,Grid,s,s_a,s_mu,n_a,n_mu,N,maxiter,tol)
...
T = length(par.Y_mat) - 1;
    ...
    a_grid = s_a;
       ...
       [c_policy(:,1),y_grid(:,1),a_policy(:,1),~] = solutionEGM_asym_nonh(par,Grid,s,s_a,s_mu,n_a,n_mu,N,maxiter,tol);
       
       par.Y = par.Y_mat(T+1);
       par.kappa = par.kappa_mat(T+1);
       par.r_star_l = par.lr_mat(T+1);
       [c_policy(:,T+1),y_grid(:,T+1),a_policy(:,T+1),~] = solutionEGM_asym_nonh(par,Grid,s,s_a,s_mu,n_a,n_mu,N,maxiter,tol);      
   ...
   for j =1:N
    if ctilde_bind(j)<=10e-6 
       ctilde_bind(j)=10e-6; % min val
    else
