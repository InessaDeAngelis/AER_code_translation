...
[c,y,ap,bind_mat] = solutionEGM_asym(par,Grid,s,s_a,s_mu,n_a,n_mu,N,maxiter,tol);
...
pers_perm = 1-0.983; % persistence g shock

run mit_shocks_r_asym.m

elast_PI_rasym_mex = elast_PI;
...
elast_PI_rasym_mex_plot = interp1(nmu_cum(8:23),elast_PI_rasym_mex(2,8:23),0.05:.1:.95,'linear','extrap');
...
hold on;
plot(1:10,elast_PI_rasym_mex_plot,'kD','LineWidth',3,'MarkerSize',12,'Color',[255/256,69/256,0/256])
hold on;
...
legend('baseline model','model w. int. shocks', 'data','Location','Northeast' );
xlim([1 10]);
...
print( gcf, '-dpdf', '-r300', 'figureD7_a');
movefile('figureD7_a.pdf',dir_fig);
