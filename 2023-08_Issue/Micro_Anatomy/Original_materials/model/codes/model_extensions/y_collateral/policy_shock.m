...
% sequence of shocks
    ...
    a_grid = s_a;
       ...
       for i = T:-1:2
           ...
           par.kappa = par.kappa_mat(i);

for j=1:n_mu
    [~,aa] = min( abs( s_a - (-par.kappa*(y_mu(((j-1)*n_a+1):((j-1)*n_a+n_a)).^par.nu_mu).*(par.Y)) ) );
...
ctilde_bind = (1+par.r_star)*s(:,1) + y_grid(:,i) + par.kappa.*(exp( s(:,2) ).^par.nu_mu).*(par.Y);
