...
data_Y_ITA = readtable('../../../input/dataY_ITA.xls');
data_smooth= [(1:9)',log(data_Y_ITA.y(12:20))];
data_Y_ITA_s=lowess(data_smooth,.7);
data_Y_ITA_ep = exp(data_Y_ITA_s(:,3));
data_Y_ITA_ep = data_Y_ITA_ep./data_Y_ITA_ep(1,1);

% -> permanent one time shock = kappa constant + sequence of aggregate income shocks

par.Y_mat = zeros(40,8);
for ii = 1:8
par.Y_mat(1:ii+1,ii) = data_Y_ITA_ep(1:1+ii);
for hh = ii+2:length(par.Y_mat)
par.Y_mat(hh,ii) = par.Y_mat(hh-1,ii).*exp(log(par.Y_mat(hh-1,ii))*pers_perm^(hh-ii-1));
end
end

par.kappa_mat = par.kappa.*ones(40,8); % remains unchanged

[c_PI,ap_PI,y_PI,a_PI] = policy_shock_simul(par,Grid,s,s_a,s_mu,n_a,n_mu,length(s),maxiter,tol);
Y_mat_PI_mat = par.Y_mat;
Y_mat_PI = par.Y_mat(:,8);
...
elast_PI_si = (log(C_dist_PI(:,:)) - log(C_dist_PI(1,:)))./(log(Y_dist_PI(:,:)) - log(Y_dist_PI(1,:)));
