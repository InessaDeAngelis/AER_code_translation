...
s_transf = repmat(par.transf,[n_a 1]);
s_transf = s_transf(:);
...
% initial guess
y = par.Y.*exp( s(:,2) );
ctilde_bind = (1+par.r_star)*s(:,1) + y + s_transf - s(1,1);
c0 = par.r_star*s(:,1) + y + s_transf;
...
atilde = (1/(1+par.r_star))*( ctilde + s(:,1) - par.Y.*exp( s(:,2) ) - s_transf );
...
c = c1;
y =  par.Y.*exp(s(:,2));
ap =  y + s_transf + (1+par.r_star).*s(:,1) - c;
