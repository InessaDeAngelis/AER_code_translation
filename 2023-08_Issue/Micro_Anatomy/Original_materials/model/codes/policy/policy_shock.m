...
T = length(par.Y_mat) - 1;
    ...
    a_grid = s_a;
       ...
       s_transf = repmat(par.transf_mat(i,:),[n_a 1]);
       s_transf = s_transf(:);
    ...
    atilde0_id(aa+(j-1)*n_a) = 1;
     ...
     ctilde_bind = (1+par.r_star)*s(:,1) + y_grid(:,i) + s_transf + par.kappa_mat(i);
   ...
   for j =1:N
    if ctilde_bind(j)<=10e-7 
       ...
       atilde = (1/(1+par.r_star))*( ctilde + s(:,1) - y_grid(:,i) - s_transf);
...
a_policy(:,i) =  y_grid(:,i) + s_transf + (1+par.r_star).*s(:,1) - c_policy(:,i);
