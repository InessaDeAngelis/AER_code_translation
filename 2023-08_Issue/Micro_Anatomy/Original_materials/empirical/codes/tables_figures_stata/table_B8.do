...
* Code replicates Table 1 panel a and Table B8 panel b
* Elasticities of durable consumption for average and top income
**********************************************************************
 ...
 *********************************************************
 ******* Table B8 panel - All Households and Top-Y *******
 *********************************************************
...
merge 1:1 nquest anno using "$input_data_ITA/storico_stata/rfam.dta"
keep if _merge == 3
drop _merge

gen consum_nd = cn - yca2  - yl2 /* non-durable and monetary */
gen consum_d = cd /* durable */

keep anno nquest consum_nd consum_d

tempfile consum_data
save `consum_data', replace

u "$database/baseline_ITA.dta", clear

merge 1:1 anno nquest using `consum_data'
keep if _merge == 3
drop _merge

merge m:1 anno using "$input_data_ITA/storico_stata/defl.dta"
keep if _merge == 3
drop _merge

*income and durable cons
replace income = income*rival

replace consum = consum_d /* durable */
...
*durable cons
gen durab = gastmon - nondurab
gen cons = durab /* durable */
...
*durable cons
gen consum = GASMON - GAS_TRI /* durable */
...
local y_beta = _b[year]

reg lc EDAD EDAD2 SEXb* TAM_HOGb* [aw=HOG] /* residualization different for durables; wrong coefficients in baseline residualization */
predict uc if e(sample),res
replace uc = uc - `y_beta'*year /* de-trend using income's trend */
...
*durable cons
gen consu = (rent_expend + hhequip_expend)/CPI_index /* durable */
...
outsheet tc* in 1/`row' using "$output/tableB8_c.tex", noquote nonames delimit(" ") replace
