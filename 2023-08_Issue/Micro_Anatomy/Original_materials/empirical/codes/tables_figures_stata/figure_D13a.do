...
* Code replicates Figure D.13 panel a
* Loadings of aggregate income volatility across income distribution
********************************************************************** 
...
*** estimate loadings of sigma_i to sigma ***
...
forval i =1/10{
reg luy luy_agg if decile == `i', robust
...
graphregion(color(white)) ylabel(, grid labsize(large)) xsize(`width_') ysize(`height_') xlabel(1(1)10, labsize(large)) ///
legend(off) xlabel(,grid) yline(0,lc(gray*.5) lp(dash) lw(.4))
graph export "$output/figureD13_a.pdf", replace
